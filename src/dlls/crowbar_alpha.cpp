...
class CCrowbarAlpha : public CBasePlayerWeapon
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( weapon_crowbar_alpha, CCrowbarAlpha );
...
void CCrowbarAlpha::Spawn( )
{
	Precache( );
	m_iId = WEAPON_CROWBAR_ALPHA;
...
void CCrowbarAlpha::Precache( void )
{
	PRECACHE_MODEL("models/v_crowbar_alpha.mdl");
...
int CCrowbarAlpha::GetItemInfo(ItemInfo *p)
{
...
BOOL CCrowbarAlpha::Deploy( )
{
	CVAR_SET_FLOAT("cl_viewmodel_fov", 90);
	return DefaultDeploy("models/v_crowbar_alpha.mdl", "models/p_crowbar.mdl", CROWBAR_DRAW, "crowbar");
}

void CCrowbarAlpha::Holster( )
{
...
void FindHullIntersectionAlpha( const Vector &vecSrc, TraceResult &tr, float *mins, float *maxs, edict_t *pEntity )
{
	int			i, j, k;
	float		distance;
...
void CCrowbarAlpha::PrimaryAttack()
{
	if (! Swing( 1 ))
	{
		SetThink(&CCrowbarAlpha::SwingAgain );
		pev->nextthink = gpGlobals->time + 0.1;
	}
}


void CCrowbarAlpha::Smack( )
{
	DecalGunshot( &m_trHit, BULLET_PLAYER_CROWBAR );
}


void CCrowbarAlpha::SwingAgain( void )
{
	Swing( 0 );
}


int CCrowbarAlpha::Swing( int fFirst )
{
	int fDidHit = FALSE;
	...
	if ( tr.flFraction >= 1.0 )
	{
		...
		if ( tr.flFraction < 1.0 )
		{
			...
			if ( !pHit || pHit->IsBSPModel() )
				FindHullIntersectionAlpha( vecSrc, tr, VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX, m_pPlayer->edict() );
	...
	if ( tr.flFraction >= 1.0 )
	{
		if (fFirst)
		{
			...
			case 3:
				SendWeaponAnim(CROWBAR_ATTACK4); break;
			case 4:
				SendWeaponAnim(CROWBAR_ATTACK5); break;
			}
